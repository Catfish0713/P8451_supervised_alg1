...
confusionMatrix(knn.model.2)

#REPEAT using sequence of k defined by user
...
model.results.3 <- test.data %>%
  predict(knn.model.3, new_data = .)

test.outcome <- test.data %>%
  dplyr::select(-outcome.class) %>%
  mutate(predicted = predict(knn.model.3, newdata = .))
